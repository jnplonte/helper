        ...
        return data.map( (row) => {
            ...
            show.forEach( (showdata) => {
                if (has(row, showdata)) {
                    if (showdata.indexOf('.') >= 1) {
                        showdata = showdata.split('.');
                        finalDataArray[showdata[0]] = {...finalDataArray[showdata[0]], ...filterData(row[showdata[0]], [showdata.slice(1).join('.')])};
                    } else {
                    ...
                    if (showdata.indexOf('.') >= 1) {
                        ...
                        if (row[showdata[0]] && row[showdata[0]].constructor === Array) {
                            ...
                            row[showdata[0]].forEach( (dataval, key) => {
                                finalDataArray[showdata[0]][key] = {...finalDataArray[showdata[0]][key], ...filterData(dataval, [showdata.slice(1).join('.')])};
                            });
                        }
                    }
                }
